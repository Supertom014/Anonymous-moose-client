...
import configparser
import zipfile
from policy_loader import LoadPolicy
import logging
...
class LoadConfig(object):
    def __init__(self, options={}, policy_list_list = []):
        ...
        if run:
            ...
            for name in namelist:
                if name == 'settings.ini':
                    ...
                    options.update({'messages': c.get_messages(), 'accounts': c.get_accounts()})
                    options.update(c.get_misc())
                elif name == 'server_public_key.pem':
                    key = settings_zip_object.open(name, mode='r', pwd=bytes('thomas', encoding='utf-8')).read().decode(encoding='utf-8')
                    options.update({'alices_public_key': key})
                else:
                    policy = settings_zip_object.open(name, mode='r', pwd=bytes('thomas', encoding='utf-8')).read()
                    policy = policy.decode(encoding='utf-8')
                    policy_list_list.append(LoadPolicy(policy, name).get_policy_list())
        self.options = options
        self.policy_list_list = policy_list_list

    def get_options(self):
        return self.options

    def get_policy_list_list(self):
        return self.policy_list_list
